package com.example.chady.ui.bluetooth
...
import com.example.chady.ui.device.Device
import com.example.chady.ui.device.DeviceAdapter
import com.example.chady.R
import com.example.chady.ui.localiser.LocaliserActivity2.Companion.PERMISSION_REQUEST_LOCATION
...
// L'Adapter permettant de se connecter
  ...
  ScanFilter.Builder().setServiceUuid(ParcelUuid(BluetoothLEManager.DEVICE_UUID)).build()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fun setupRecycler() {
            val rvDevice =
                findViewById<RecyclerView>(R.id.rvDevices) // Récupération du RecyclerView présent dans le layout
    ...
    private val leScanCallback: ScanCallback = object : ScanCallback() {
        ...
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            ...
            // C'est ici que nous allons créer notre « Device » et l'ajouter dans la dataSource de notre RecyclerView
             ...
             val device =
                 Device(result.device.name, result.device.address, result.device)
    ...
    private fun handleToggleLedNotificationUpdate(characteristic: BluetoothGattCharacteristic) {
        if (characteristic.getStringValue(0).equals("1", ignoreCase = true)) {
            ledStatus?.setImageResource(R.drawable.ampouleallumee)
        } else {
            ledStatus?.setImageResource(R.drawable.ampouleeteinte)
        }
    }
